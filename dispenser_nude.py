...
def dispense(start_weight):
    ...
    while GPIO.input(START) == GPIO.HIGH and time.time() < timeout_time:

        time.sleep(0.1)
        
        current_weight1 = hx.get_weight(5) - tare
        current_weight2 = hx.get_weight(5) - tare
        current_weight3 = hx.get_weight(5) - tare
        
        current_weight = (current_weight1+current_weight2+current_weight3)/3
    ...
    if GPIO.input(START) != GPIO.HIGH:
            ...
            beepbeep()
            ledoff()
            printer(str(rands), str(int(dispensed)))
...
def main():
    ...
    lcd_string("Total: " + current_weight + "g",LCD_LINE_4,2)
    ...
    if GPIO.input(START) == GPIO.HIGH:                    ## START pushed                   
        beepbeep()
        ledon()
        #logging.info('\tFree Mode; \t-; \t-; \t' + logstring1)
        
        start_weight1 = hx.get_weight(5) - tare
        start_weight2 = hx.get_weight(5) - tare
        start_weight3 = hx.get_weight(5) - tare
        
        start_weight = (start_weight1+start_weight2+start_weight3)/3
